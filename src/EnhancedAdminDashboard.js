...
import BoothActivityExporter from "./BoothActivityExporter";
...
const EnhancedAdminDashboard = ({ onCreateEvent, onLogout }) => {
  ...
  const softDeleteEvent = async (eventId) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this event? It will be moved to trash and permanently deleted after 30 days."
      )
    ) {
      return;
    }
  ...
  // Enhanced renderEventActions function with export functionality
      ...
      return (
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 text-green-600 hover:text-green-800 px-2 py-1 bg-green-100 rounded text-xs font-medium"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 text-red-600 hover:text-red-800 px-2 py-1 bg-red-100 rounded text-xs font-medium"
          >
            <Trash2 className="w-3 h-3" />
            Delete Forever
          </button>
          <div className="text-xs text-gray-500">
            Deletes in {getDaysUntilDeletion(event.deleted_at)} days
          </div>
        </div>
      );
    }
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 text-purple-600 hover:text-purple-800 px-2 py-1 bg-purple-100 rounded text-xs font-medium"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 text-blue-600 hover:text-blue-800 px-2 py-1 bg-blue-100 rounded text-xs font-medium"
          >
        ...
        {/* Export Activity Button - Only for booth mode events */}
        {event.access_type === "email_only" && (
          <div className="pt-2 border-t border-gray-100">
            <BoothActivityExporter
              eventId={event.id}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  <span>
                    Welcome back, {user.display_name || user.eventbrite_email}
                  </span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-4 h-4" />
                    Last login: {formatLastLogin(user.last_login)}
                  </span>
                  {user.login_count && <span>Login #{user.login_count}</span>}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <h2 className="text-xl font-bold text-gray-900">
                  {showDeleted ? "Deleted Events" : "Your Events"} (
                  {showDeleted ? deletedEvents.length : events.length})
                </h2>
                <button
            ...
            {showDeleted && deletedEvents.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">
                  Deleted events will appear here and be permanently removed
                  after 30 days.
                </p>
              </div>
            )}
