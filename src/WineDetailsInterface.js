...
const WineDetailsInterface = ({
  ...
  const [userRating, setUserRating] = useState(currentRating?.rating || 0);
  const [ratingNotes, setRatingNotes] = useState(currentRating?.notes || "");
  ...
  useEffect(() => {
    if (currentRating) {
      ...
      setRatingNotes(currentRating.notes || "");
    }
  }, [currentRating]);
  ...
  const AnimatedBar = ({ grape, index }) => {
    ...
    return (
      <div className="mb-4 last:mb-0">
        ...
        <div
          className={`h-3 rounded-full ${theme.accentLight} overflow-hidden`}
        >
          <div
  ...
  // Get or create user profile based on session
  const getOrCreateUserProfile = async () => {
    ...
    try {
      ...
      const { data: existingUser } = await supabase
        .from("profiles")
        .select("*")
        .eq("eventbrite_email", userSession.email.toLowerCase())
      ...
      const newProfile = {
        ...
        display_name: userSession.email.split("@")[0], // Use email prefix as display name
        eventbrite_email: userSession.email.toLowerCase(),
  ...
  const saveRating = async () => {
    ...
    try {
      ...
      if (isExpertMode && selectedDescriptors.length > 0) {
        // First, remove existing descriptors for this rating
        await supabase
          .from("user_wine_descriptors")
          .delete()
          .eq("user_rating_id", ratingData.id);
      ...
      // Call the callback with the rating data
      onRatingSaved({
        rating: userRating,
        notes: ratingNotes,
      });
  ...
  return (
    <div className="min-w-full">
      ...
      >
        {/* Header */}
        <div
          className={`${theme.cardBg} ${theme.border} border-b p-4 sm:p-5 sticky top-0 z-10 backdrop-blur-sm bg-opacity-95`}
        >
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1
                  className={`text-lg sm:text-xl font-semibold ${theme.text}`}
                >
        ...
        <div className="max-w-4xl mx-auto">
          {/* Wine Image */}
          <div
            className={`relative h-64 sm:h-80 lg:h-96 bg-gradient-to-br ${theme.gradient}`}
          >
            {wine.image_url ? (
              <img
                src={wine.image_url}
            ...
            <div
              ...
              style={{ display: wine.image_url ? "none" : "flex" }}
            >
              <div className="text-6xl sm:text-7xl lg:text-8xl filter drop-shadow-lg">
                {terminology.icon}
              </div>
            </div>
            <div className="absolute top-4 sm:top-5 left-4 sm:left-5 right-4 sm:right-5 flex justify-between">
              <div
                className={`${theme.cardBg} rounded-xl px-3 sm:px-4 py-2 backdrop-blur-sm bg-opacity-90 shadow-lg`}
              >
          ...
          <div
            className={`${theme.cardBg} mx-4 sm:mx-5 lg:mx-6 mt-4 sm:mt-5 rounded-2xl p-4 sm:p-6 ${theme.border} border shadow-sm`}
          >
            <div className="mb-4 sm:mb-5">
              <h2
                className={`text-xl sm:text-2xl lg:text-3xl font-bold ${theme.text} mb-2`}
              >
                {wine.wine_name}
              </h2>
              ...
              {wine.awards && wine.awards.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {wine.awards.slice(0, 2).map((award, index) => (
                    <span
                      key={index}
                      className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-full bg-amber-100 text-amber-800"
                    >
                      <Award size={12} />
                      {award}
                    </span>
              ...
              {wine.wine_style && wine.wine_style.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {wine.wine_style.map((style, index) => (
                    <span
                      key={index}
                      className={`px-3 py-1.5 text-sm rounded-full ${theme.accentLight} ${theme.accentText} font-medium`}
                    >
                      {style}
                    </span>
                  ))}
            ...
            <p className={`${theme.textSecondary} leading-relaxed text-base`}>
              {wine.sommelier_notes ||
                "A carefully selected wine for this tasting experience."}
            </p>
          </div>
            ...
            <div className="lg:grid lg:grid-cols-2 lg:gap-6 space-y-4 lg:space-y-0">
              ...
              <div className="space-y-4">
                ...
                {wine.grape_varieties && wine.grape_varieties.length > 0 && (
                  <div
                    className={`${theme.cardBg} rounded-2xl ${theme.border} border overflow-hidden shadow-sm`}
                  >
                    <button
                    ...
                    >
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div
                          className={`p-2 rounded-lg bg-gradient-to-br from-amber-500 to-amber-600 text-white`}
                        >
                          <Grape size={20} />
                        </div>
                        <div className="text-left">
                          <span
                            className={`font-semibold ${theme.text} block text-base`}
                          >
                            {terminology.grapeSection}
                          </span>
                          <span className={`text-sm ${theme.textMuted}`}>
                            {terminology.grapeSubtext}
                          </span>
                        </div>
                      </div>
                      <div
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        className="transition-transform duration-300"
                        style={{
                          transform: expandedSections.winemaker
                            ? "rotate(180deg)"
                            : "rotate(0deg)",
                        }}
                      >
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        <div className="text-left">
                          ...
                          <span className={`text-sm ${theme.textMuted}`}>
                            Sommelier analysis & characteristics
                          </span>
                        </div>
                      </div>
                      <div
                        className="transition-transform duration-300"
                        style={{
                          transform: expandedSections.sommelier
                            ? "rotate(180deg)"
                            : "rotate(0deg)",
                        }}
                      >
                    ...
                    >
                      <div className="px-4 sm:px-5 pb-4 sm:pb-5 space-y-4">
                        {wine.sommelier_notes && (
                          <div
                            className={`${theme.textSecondary} italic border-l-4 border-amber-200 pl-4 py-2 bg-amber-50 rounded-r-lg`}
                          >
                            "{wine.sommelier_notes}"
                          </div>
                        )}
                        ...
                        {wine.tasting_notes && (
                          <div className="grid gap-4">
                            {wine.tasting_notes.appearance && (
                              <div
                                className={`p-4 rounded-xl ${theme.accentLight} border ${theme.border}`}
                              >
                                <h4
                      ...
                      <div className="px-4 sm:px-5 pb-4 sm:pb-5 space-y-3">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                          {wine.technical_details.ph && (
                            <div
                              className={`p-3 rounded-lg ${theme.accentLight} border ${theme.border}`}
                            >
                              <span
                                className={`block font-semibold ${theme.text} mb-1`}
                              >
                                pH Level
                              </span>
              ...
              >
                ...
                >
                  ...
                  <div
                    className="transition-transform duration-300"
                    style={{
                      transform: expandedSections.foodPairing
                        ? "rotate(180deg)"
                        : "rotate(0deg)",
                    }}
                  >
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                          {category.items.map((item, itemIndex) => (
                            <div
                              key={itemIndex}
                              className={`p-3 text-center rounded-lg ${theme.cardBg} ${theme.border} border hover:shadow-sm transition-shadow`}
                            >
                              <span
          ...
          <div
            className={`${theme.cardBg} mx-4 sm:mx-5 lg:mx-6 mt-4 sm:mt-5 mb-6 sm:mb-8 rounded-2xl p-4 sm:p-6 ${theme.border} border shadow-sm`}
          >
            ...
            {userRating > 0 && (
              <div
                className={`text-center mb-4 p-3 rounded-xl ${theme.accentLight} border ${theme.border}`}
              >
            ...
            {isExpertMode && (
              <div className="mb-6 space-y-4">
                <h4
                  className={`font-medium ${theme.text} flex items-center gap-2`}
                >
                  <Edit3 size={16} />
                  Tasting Descriptors
                </h4>

                {Object.entries(descriptorData).map(
                  ([category, subcategories]) => (
                    ...
                    >
                      <h5
                        className={`font-medium ${theme.text} mb-3 capitalize`}
                      >
                        {category}
                      </h5>
                      {Object.entries(subcategories).map(
                        ([subcat, descriptors]) => (
                          <div key={subcat} className="mb-3 last:mb-0">
                            <p
                              className={`text-xs ${theme.textMuted} mb-2 capitalize font-medium`}
                            >
                              {subcat}
                            </p>
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-semibold transition-all duration-200 transform hover:scale-105 ${theme.accent} ${theme.accentHover} text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none`}
            >
              {saving
                ? "Saving..."
                : currentRating
                ? "Update Rating & Continue"
                : "Submit Rating & Continue to Next Wine"}
            </button>
          </div>
        </div>
      </div>
